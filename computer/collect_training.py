...
class CollectTrainingData(object):
    def __init__(self):
        ...
        self.server_socket.bind(('192.168.0.107', 8000))
    ...
    def collect_imgdata(self):
        ...
        try:
            ...
            while self.isReceiving:
                stream_bytes += str(self.connection.read(1024))
                    ...
                    for event in pygame.event.get():
                        if event.type == KEYDOWN:
                            ...
                            if keypress == pygame.K_LEFT:
                                print('left')
                                urllib.urlopen('http://192.168.0.107:5000/left').read()
                                ...
                                print('right')
                                urllib.urlopen('http://192.168.0.107:5000/right').read()
                                image_array = np.vstack((image_array, temp_image_array))
                                ...
                                print('forward')
                                urllib.urlopen('http://192.168.0.107'
                                                ':5000/forward').read()
                                image_array = np.vstack((image_array, temp_image_array))
